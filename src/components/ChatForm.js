...
const CharCountContainer = styled.div`
  ...
  opacity: ${props => props.beganTyping || 0}
  margin: 0 auto
  grid-column: 1;
  grid-row: 1;
`;
...
class ChatForm extends React.Component {
  constructor(props) {
    ...
    // his.handleEmailChange = this.handleEmailChange.bind(this);
  ...
  */
  ...
  handleTextAreaChange(event) {
    ...
    this.setState({
      ...
      remaining: maxLength - event.target.value.length,
    });
  }
  ...
  render() {
    const { maxLength, remaining, message } = this.state;
        ...
        <div style={{ display: 'grid' }}>
          ...
          >
            <h2>
              ...
              <CharLimit
                status={maxLength - message.length}
                style={{
